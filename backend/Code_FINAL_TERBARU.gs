...
function doGet(e) {
  ...
  if (action === 'aspelStatusAdminSync') {
    let result;
...
function cbtAccountScheduleFast_(email, uid) {
  const book = SpreadsheetApp.openById(PN_BIODATA_SPREADSHEET_ID);
  const accountSheet = book.getSheetByName(PN_PORTAL_ACCOUNT_SHEET_NAME);
  if (!accountSheet) throw new Error('Sheet Akun Portal Siswa tidak ditemukan.');
  ...
  if (lastAccount >= 2) {
    ...
    for (let i=0;i<rows.length;i++) {
      ...
      if (wantedUid && candidate.uid === wantedUid) {
        account = candidate;
        break;
      }
  ...
  if (!account) {
    if (matchCount > 1) {
      throw new Error('Email terhubung ke lebih dari satu akun CBT. Hubungi admin untuk merapikan database akun.');
    }
    account = fallback;
  }
...
function cbtAccessCheck_(data) {
  const startedAt = Date.now();
  const idToken = String(data.idToken || '').trim();
  if (!idToken) throw new Error('Sesi login CBT wajib tersedia.');
  ...
  const dataFast = cbtAccountScheduleFast_(email, uid);
  const account = dataFast.account;
  if (!account) {
    return {ok:true,allowed:false,code:'ACCOUNT_NOT_FOUND',message:'Email ini belum terdaftar sebagai akun CBT. Hubungi admin.'};
  }
  ...
  if (account.uid && account.uid !== uid) {
    return {ok:true,allowed:false,code:'UID_MISMATCH',message:'Email ini sudah terhubung dengan pengguna lain. Hubungi admin.'};
  }
  if (!account.memberId) {
    return {ok:true,allowed:false,code:'MEMBER_ID_MISSING',message:'ID Anggota akun ini belum terhubung. Hubungi admin.'};
  }
  if (!account.username) {
    return {ok:true,allowed:false,code:'USERNAME_MISSING',message:'Username internal akun belum tersedia. Hubungi admin.'};
  }
  if (!account.uid) {
...
function aspelStatusAdminSync_(data) {
  const admin = requireReviewAdmin_(data.token);
