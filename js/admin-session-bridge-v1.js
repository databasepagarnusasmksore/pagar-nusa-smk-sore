...
const TOKEN_KEY='pnReviewAdminToken',DEVICE_KEY='pnAdminDeviceSessionV1',SOURCE='pn-content';
let pending=null;
function currentToken(){try{return sessionStorage.getItem(TOKEN_KEY)||localStorage.getItem(TOKEN_KEY)||localStorage.getItem(DEVICE_KEY)||''}catch(_){return''}}
function saveToken(v){const t=String(v||'');try{sessionStorage.setItem(TOKEN_KEY,t)}catch(_){};try{localStorage.setItem(TOKEN_KEY,t);localStorage.setItem(DEVICE_KEY,t)}catch(_){}}
function clearToken(){try{sessionStorage.removeItem(TOKEN_KEY)}catch(_){};try{localStorage.removeItem(TOKEN_KEY);localStorage.removeItem(DEVICE_KEY)}catch(_){}}
...
window.pnCurrentAdminServerTokenV1=currentToken;
window.pnAdminServerSessionNeedsLoginV1=function(){
  let auth=false;try{auth=localStorage.getItem('pnAdminAuth')==='1'||sessionStorage.getItem('pnAdminAuth')==='1'}catch(_){}
  return !!(auth&&!currentToken()&&!(window.__pnAdminEphemeralCredentials&&window.__pnAdminEphemeralCredentials.password));
};
